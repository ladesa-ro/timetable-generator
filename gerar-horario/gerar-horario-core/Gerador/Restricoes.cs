...
public class Restricoes
{
    ...
    {
        for (
            int diaSemanaIso = options.DiaSemanaInicio;
            diaSemanaIso <= options.DiaSemanaFim;
            diaSemanaIso++
        )
        {
            for (
                int intervaloIndex = 0;
    ...
    {
        foreach (
            var combinacao in Restricoes.GerarTodasAsCombinacoesPossiveisInclusiveIndisponiveis(
                options
            )
        )
        {
            ...
            var disponivelParaTurma = Restricoes.VerificarIntervaloEmDisponibilidades(
                turma.Disponibilidades,
            ...
            var disponivelParaProfessor = Restricoes.VerificarIntervaloEmDisponibilidades(
    ...
    ///</summary>
    public static void AplicarLimiteDeDiarioNaSemana(GerarHorarioContext contexto)
    {
    ...
    ///</summary>
    public static void AplicarLimiteDeNoMaximoUmDiarioAtivoPorTurmaEmUmHorario(
    ...
    )
    {
        ...
        {
            foreach (
                var intervaloIndex in Enumerable.Range(0, contexto.Options.HorariosDeAula.Length)
            )
            {
                foreach (var turma in contexto.Options.Turmas)
                {
    ...
    ///</summary>
    public static void AplicarLimiteDeNoMaximoUmDiarioAtivoPorProfessorEmUmHorario(
        GerarHorarioContext contexto
    )
    {
        foreach (var professor in contexto.Options.Professores)
        {
            ...
            {
                foreach (
                    var intervaloIndex in Enumerable.Range(
                        0,
                        contexto.Options.HorariosDeAula.Length
                    )
                )
                {
                    var propostas =
                        from propostaDeAula in contexto.TodasAsPropostasDeAula
                        where
                            propostaDeAula.DiaSemanaIso == diaSemanaIso
                            && propostaDeAula.IntervaloIndex == intervaloIndex
                            && contexto.Options.ProfessorEstaVinculadoAoDiario(
                                diarioId: propostaDeAula.DiarioId,
            ...
            )
            {
                ...
                var propostasTarde =
                    ...
                    where
                        proposta.ProfessorId == professor.Id
                        && proposta.DiaSemanaIso == diaSemanaIso
                        && proposta.IntervaloIndex >= 5
                        && proposta.IntervaloIndex <= 9
                ...
                */
                if (propostasManha.Any() && propostasTarde.Any() && propostasNoite.Any())
                {
                    ...
                    var qntAulasManha = contexto.Model.NewIntVar(
                        ...
                        $"qnt_ativo_{professor.Id}_{diaSemanaIso}_Manha"
                    );
                    var qntAulasTarde = contexto.Model.NewIntVar(
                        0,
                        propostasTarde.Count(),
                        $"qnt_ativo_{professor.Id}_{diaSemanaIso}_Tarde"
                    );
                    var qntAulasNoite = contexto.Model.NewIntVar(
                        0,
                        propostasNoite.Count(),
                        $"qnt_ativo_{professor.Id}_{diaSemanaIso}_Noite"
                    );
                    ...
                    var alumgaAulaManha = contexto.Model.NewBoolVar(
                        $"ativo_{professor.Id}_{diaSemanaIso}_Manha"
                    ); // == LinearExpr.Sum(propostasManha) > 0;
                    var alumgaAulaTarde = contexto.Model.NewBoolVar(
                        $"ativo_{professor.Id}_{diaSemanaIso}_Tarde"
                    ); // == LinearExpr.Sum(propostasTarde) > 0;
                    var alumgaAulaNoite = contexto.Model.NewBoolVar(
                        $"ativo_{professor.Id}_{diaSemanaIso}_Noite"
                    ); // == LinearExpr.Sum(propostasNoite) > 0;
                    ...
                    contexto
                        .Model.AddAllowedAssignments(
                            [alumgaAulaManha, alumgaAulaTarde, alumgaAulaNoite]
                        )
            ...
            foreach (var professor in contexto.Options.Professores)
            {
                ...
                foreach (var propostaNoite in propostasNoite)
                {
                    ...
                    var propostasConflitantesManhaSeguinte =
                        from proposta in contexto.TodasAsPropostasDeAula
                        where
                            proposta.DiaSemanaIso == diaSemanaIsoSeguinte
    ...
    ///</summary>
    public static void AgruparDisciplinasParametro(
        GerarHorarioContext contexto,
        string professorId,
        int diaSemana,
    ...
    )
    {
        ...
        var propostasExcluir =
            from proposta in contexto.TodasAsPropostasDeAula
            where proposta.ProfessorId == professorId && proposta.DiaSemanaIso != diaSemana
            select proposta.ModelBoolVar;

        var propostasEscolhidas =
            from proposta in contexto.TodasAsPropostasDeAula
            where
                proposta.ProfessorId != professorId
    ...
    public static void PadronizarPRD(GerarHorarioContext contexto)
    {
        foreach (var professor in contexto.Options.Professores)
        {
            ...
            var propostasSorteada =
                from proposta in contexto.TodasAsPropostasDeAula
                where proposta.ProfessorId == professor.Id && proposta.DiaSemanaIso == diaSorteado
                select proposta.ModelBoolVar;
    ...
    ///</summary>
    public static void EspecificarPRD(
        GerarHorarioContext contexto,
        string idProfessor,
        int DiaSemanaEscolhido
    )
    {
        foreach (var professor in contexto.Options.Professores)
        {
        ...
        var propostasSorteada =
            from proposta in contexto.TodasAsPropostasDeAula
            where proposta.ProfessorId == idProfessor && proposta.DiaSemanaIso == DiaSemanaEscolhido
            select proposta.ModelBoolVar;

        Console.WriteLine(
            "o PRD do professor " + idProfessor + " foi escolhido para ser no " + DiaSemanaEscolhido
        );
        var negatedVariables = propostasSorteada.Select(v => v.Not()).ToArray();
        contexto.Model.AddBoolAnd(negatedVariables);
    }
