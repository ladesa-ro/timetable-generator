...
)
{
    ...
    private RecurrencePattern? GetEffectiveRecurrencePattern()
    {
        if (!string.IsNullOrEmpty(RRule) && RecurrencePattern is null)
            throw new Ladesa.TimetableGenerator.Domain.Generator.GeneratorValidationException(
                Ladesa.TimetableGenerator.Domain.Generator.GeneratorValidationErrorCode.InvalidRRule,
    ...
    public bool IsAvailable(DateOnly checkDate, TimeSlot checkTimeSlot)
    {
        ...
        return occurrences.All(occ => !HasConflict(checkStart, checkEnd, occ));
    }

    private bool HasConflict(DateTime checkStart, DateTime checkEnd, Occurrence occurrence)
    {
